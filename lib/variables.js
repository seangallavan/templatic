...
module.exports.getTemplateVariables = function getTemplateVariables(templateGroup, templateName, constraints, scope, vars) {
  ...
  ['environment', 'container'].forEach(type => {
    if (constraints[type]) {
      console.log('q', type, constraints[type]);
      console.log(JSON.stringify(vars.allApplicationsByName[constraints.application]))
      console.log(JSON.stringify(vars.allApplicationsByName[constraints.application][`${type}sByName`]))
