 ...
 */
module.exports.renderTemplate = function renderTemplate(templateGroup, templateName, scope, vars) {
  ...
  return module.exports.testing.renderTemplateRecursive(templateGroup, templateName, directoryHierarchy, 0, directoryToSaveTo, {}, scope, vars);
};
 ...
 * @return {undefined}
 */
function renderTemplateRecursive(templateGroup, templateName, directoryHierarchy, directoryLevel, directoryToSaveTo,
  ...
  if(directoryHierarchy.length === directoryLevel) {
    ...
    if(!!constraints.application && !!constraints.container
      && !vars.allApplicationsByName[constraints.application].containers.map(resource => resource.name).includes(constraints.container)) {
      return;
    }
    if(!!constraints.application && !!constraints.environment
      && !vars.allApplicationsByName[constraints.application].environments.map(resource => resource.name).includes(constraints.environment)) {
      return;
    }
    ...
    scope[`${directory}s`].forEach(targetName => {
      ...
      module.exports.testing.renderTemplateRecursive(templateGroup, templateName,
...
function renderTemplateInstance(templateGroup, templateName, renderVars) {
  ...
  return nunjucks[templateGroup].render(`${templateName}.j2`, renderVars);
}
