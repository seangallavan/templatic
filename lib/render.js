...
function init() {
  ...
  data.getTemplateTypes().forEach(templateType => {
    if (!nunjucks[templateType]) {
      nunjucks[templateType] = {};
    }

    data.getTemplateGroupNames(templateType).forEach(templateGroup => {
      if (!nunjucks[templateType][templateGroup]) {
        nunjucks[templateType][templateGroup] = new Nunjucks.Environment(new Nunjucks.FileSystemLoader(`${config.get('dataRepo:path')}/input/templates/${templateType}/${templateGroup}`));
      }
    });
  });
}
 ...
 */
module.exports.renderTemplate = function renderTemplate(templateType, templateGroup, templateName, vars) {
  init();

  const templateRenderVars = getTemplateRenderVars(templateGroup, templateName, vars);

  return nunjucks[templateType][templateGroup].render(templateName, templateRenderVars);
};
