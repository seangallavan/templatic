...
module.exports.enrichRenderVars = function enrichRenderVars(templateVars, templateGroup, templateName, directoryHierarchy, scope) {
  ...
  //   const inputs = _.get(templateVars, `${data.getGlobalTypeForInput(inputType)}ByName`, {});
  //   Object.keys(inputs).forEach(inputName => {
  //     if(inputType === 'container') {
  //       const template = inputs[inputName];
  //       const containerVars = yaml.safeLoad(nunjucks[templateGroup].renderString(template, renderVars));
  //
  //       //Add templates and template properties
  //       containerVars.template = getForEnvironment(_.get(containerVars, ['templates', templateGroup, templateName], {}));
  //       _.set(containerVars, ['templates', templateGroup, templateName], containerVars.template);
  //
  //       renderVars[data.getRenderVarsTypeForInput(inputType)].push(containerVars);
  //       renderVars[`${data.getRenderVarsTypeForInput(inputType)}ByName`][inputName] = containerVars;
  //
  //       // if (_.some(templateVars.renderVars[`${inputType}s`])) {
  //       //   templateVars.renderVars[`${inputType}s`].push(containerVars);
  //       //   templateVars.renderVars[`${inputType}sByName`] = {};
  //     } else {
  //       renderVars[data.getRenderVarsTypeForInput(inputType)].push(_.get(templateVars, [`${data.getGlobalTypeForInput(inputType)}`, inputName]));
  //       renderVars[`${data.getRenderVarsTypeForInput(inputType)}ByName`][inputName] = _.get(templateVars, [`${data.getGlobalTypeForInput(inputType)}`, inputName]);
  //     }
  //
  //     //Add to renderVars.containers, etc.
  //     if(!directoryHierarchy.includes(inputType)) {
  //       renderVars[`${inputType}s`].push(renderVars[`${data.getRenderVarsTypeForInput(inputType)}ByName`][inputName]);
  //       renderVars[`${inputType}sByName`][inputName] = renderVars[`${data.getRenderVarsTypeForInput(inputType)}ByName`][inputName];
  //     }
  //   });
  // });
};
